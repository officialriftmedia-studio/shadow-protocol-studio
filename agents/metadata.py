#!/usr/bin/env python3
"""Agent: Metadata — generates YouTube metadata (title, description, tags)."""
...
class MetadataAgent(AgentBase):
    ...
    def run(self) -> int:
        ...
        prompt = self.load_prompt("metadata")
            ...
            try:
                result = self.run_with_retry(
                    lambda: self._call_llm_with_validation(
                        ...
                        user_prompt=f"Generate YouTube metadata for this episode:\n\n{context}",
                    ),
        ...
        self.write_json("youtube_metadata.json", result)
        self.save_checkpoint(stage)
        print(f"  Wrote youtube_metadata.json")
        return 0
    ...
    def _call_llm_with_validation(
        ...
        schema_path = self.root_dir / "templates" / "schemas" / "youtube_metadata.json"
...
def run(input_dir: str, output_dir: str, config: dict) -> int:
    agent = MetadataAgent(input_dir, config)
    return agent.run()
